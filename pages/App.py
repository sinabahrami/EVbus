...
def generate_transit_report(
    ...
    story.append(Spacer(1, 12))
    add_table_from_dataframe(outputs.get('block_general'), caption_text="Summary of blocks.", styles=styles,col_width=[(page_width - 3*inch)*.2, (page_width - 3*inch)*.2, (page_width - 3*inch)*.6],  columns=["block_id", "total_distance_miles", "routes_in_block_id"], headers=["Block ID", "Total Distance (miles)", "Routes in the Block"])
