...
def compute_range_tracking(distances, time_gaps):
...
if st.button("Run Analysis"):
    ...
    if selected_agency=="Kalamazoo Metro Transit":
        weekday_service_id=9
    else:
    ...
    # Check if 'route_short_name' exists in the DataFrame
    if 'route_short_name' not in trip_distances.columns:
        raise KeyError("'route_short_name' column is missing in the merged DataFrame.")
    ...
    block_general["range_tracking"] = block_general.apply(
        lambda row: compute_range_tracking(row["distances_list"], row["time_gaps"]),
        axis=1
    )

    # Optionally, rounding values and save to CSV
    block_general["distances_list"] = block_general["distances_list"].apply(
        lambda lst: [round(x, 1) for x in lst] if isinstance(lst, list) else lst
    )

    block_general["range_tracking"] = block_general["range_tracking"].apply(
        lambda lst: [round(x, 1) for x in lst] if isinstance(lst, list) else lst
    )
    ...
    # Optionally, you can display the map or output data (depending on your needs)
    # Add any visualization or further output as needed, e.g. displaying the map:
...
if "map" in st.session_state:
    ...
    st.write(f"Minimum bus range without charging to cover all blocks is {st.session_state['miminimum_range_without_charger']} miles.")
